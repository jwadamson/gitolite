...
sub non_core_expand {
    ...
    for my $e ( @{ $rc{ENABLE} } ) {
        ...
        # store name+args as the hash value for the name
        $enable{$name} = $e;
    ...
    for (@data) {
        ...
        # module to call is name+args if specified as "."
        $module = $enable{$name} if $module eq ".";

        # if "::", you need to expand "name" into "name::pre_git" or such
        ( $module = $enable{$name} ) =~ s/(?: |$)/"::" . lc($where)/e if $module eq '::';
